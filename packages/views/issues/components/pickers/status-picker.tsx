...
export function StatusPicker({
  ...
  return (
    ...
    >
      {options.map((option) => {
        // Fork: a caller can block a specific status with an explanation
        // (e.g. a UI-design issue that has no delivered design yet).
        const disabledReason = disabledStatuses?.[option.key];
        return (
        <PickerItem
          key={option.key}
