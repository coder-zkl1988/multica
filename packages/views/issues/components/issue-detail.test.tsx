...
function renderIssueDetailWithDoneAction(issueId = "issue-1", onDone = vi.fn()) {
  const queryClient = createTestQueryClient();
  const result = render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <QueryClientProvider client={queryClient}>
...
describe("IssueDetail (shared)", () => {
  ...
  it("allows ordinary completion regardless of a design title or legacy role metadata", async () => {
    ...
    const doneButton = await screen.findByRole("button", { name: "Mark as done" });
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(mockApiObj.updateIssue).toHaveBeenCalledWith("issue-1", expect.objectContaining({ status: "done" }));
    });
  ...
  it("keeps execution in an agent block before and after its persisted reply arrives", async () => {
    ...
    await waitFor(() => expect(client.getQueryData([...issueKeys.tasks("issue-1"), "ws-1"])).toEqual([task]));
    ...
    act(() => client.setQueryData([...issueKeys.tasks("issue-1"), "ws-1"], [running]));
    ...
    act(() => client.setQueryData([...issueKeys.tasks("issue-1"), "ws-1"], [completed]));
  ...
  it.each([null, "comment-1"])("shows an assignment reply once in its run slot when posted under %s", async (parentId) => {
    ...
    act(() => queryClient.setQueryData([...issueKeys.tasks("issue-1"), "ws-1"], [running]));
    await screen.findByText("pnpm test");
    ...
    mockApiObj.listTasksByIssue.mockResolvedValue([completed]);
    act(() => queryClient.setQueryData([...issueKeys.tasks("issue-1"), "ws-1"], [completed]));
  ...
  it("replaces each queued run in place without moving replies behind later requests", async () => {
    ...
    for (const index of [0, 1]) {
      ...
      mockApiObj.listTimeline.mockResolvedValue(timeline);
      act(() => {
        client.setQueryData([...issueKeys.tasks("issue-1"), "ws-1"], [...tasks]);
