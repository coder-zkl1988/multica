...
const ProjectDesignSystemPreviewValidationSchema = z.preprocess(
  (value) => value == null ? {} : value,
  z.object({
    status: z.enum(["none", "pending", "passed", "failed"]).catch("none").default("none"),
    integrity_sha256: z.string().catch("").default(""),
...
const ProjectRepositoryDesignFactSchema = z.object({
  ...
  confidence: z.number().min(0).max(1).catch(0).default(0),
}).loose();
...
const ProjectDesignSystemInputSnapshotSchema = z.preprocess(
  ...
  z.object({
    ...
    brief: z.string().catch("").default(""),
    references: z.preprocess(
      (value) => value == null ? [] : value,
      z.array(z.record(z.string(), z.unknown())).catch([]),
    ),
    repository_analysis: ProjectRepositoryDesignContextSchema.optional().catch(undefined),
  }).loose(),
);
