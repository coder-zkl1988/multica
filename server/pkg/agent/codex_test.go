...
func TestCodexRawItemMCPToolCall(t *testing.T) {
	...
	// This fork's redact.go additionally short-circuits on the field name
	// itself (secretFieldNamePattern) before falling through to value-shape
	// matching. "credentials" itself matches that pattern, so the whole
	// sub-map collapses to the generic placeholder string rather than a map
	// with "api_key" redacted individually to the sk-prefix-specific marker
	// upstream expects.
	if arguments["credentials"] != "[REDACTED CREDENTIAL]" {
