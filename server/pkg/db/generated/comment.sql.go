...
const bindDesignDeliveryTask = `-- name: BindDesignDeliveryTask :one
UPDATE agent_task_queue SET trigger_comment_id = $1,
    originator_user_id = $2, accountable_user_id = $2,
    originator_source = 'direct_human', trigger_evidence_kind = 'comment',
    trigger_evidence_ref_id = $1, trigger_summary = $3
WHERE id = $4 AND issue_id = $5
...
type BindDesignDeliveryTaskParams struct {
...
func (q *Queries) BindDesignDeliveryTask(ctx context.Context, arg BindDesignDeliveryTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, bindDesignDeliveryTask,
		arg.CommentID,
		arg.RequesterID,
		arg.TriggerSummary,
		arg.ID,
...
WHERE id = $1
  AND workspace_id = $2
RETURNING id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery
`
...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery FROM comment
WHERE id = $1
`
...
const getCommentByDesignRequest = `-- name: GetCommentByDesignRequest :one
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery FROM comment
WHERE workspace_id = $1 AND author_type = $2 AND author_id = $3
  AND design_delivery->>'request_id' = $4::text
`

type GetCommentByDesignRequestParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	AuthorType  string      `json:"author_type"`
	AuthorID    pgtype.UUID `json:"author_id"`
...
func (q *Queries) GetCommentByDesignRequest(ctx context.Context, arg GetCommentByDesignRequestParams) (Comment, error) {
	row := q.db.QueryRow(ctx, getCommentByDesignRequest,
		arg.WorkspaceID,
		arg.AuthorType,
		arg.AuthorID,
		arg.RequestID,
...
SELECT c.id, c.issue_id, c.author_type, c.author_id, c.content, c.type, c.created_at, c.updated_at, c.parent_id, c.workspace_id, c.resolved_at, c.resolved_by_type, c.resolved_by_id, c.source_task_id, c.quick_action_id, c.via_plugin_id, c.revision, c.recovery_settled_at, c.deleted_at, c.design_delivery FROM comment c
...
WITH RECURSIVE ancestor_path AS (
  ...
  SELECT parent.id, parent.issue_id, parent.author_type, parent.author_id, parent.content, parent.type, parent.created_at, parent.updated_at, parent.parent_id, parent.workspace_id, parent.resolved_at, parent.resolved_by_type, parent.resolved_by_id, parent.source_task_id, parent.quick_action_id, parent.via_plugin_id, parent.revision, parent.recovery_settled_at, parent.deleted_at, parent.design_delivery,
...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery
FROM thread_history
...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery FROM (
    SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery FROM comment
    WHERE issue_id = $1 AND workspace_id = $2
...
const setCommentDesignDelivery = `-- name: SetCommentDesignDelivery :one
UPDATE comment SET design_delivery = $1
WHERE id = $2 AND workspace_id = $3
RETURNING id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, design_delivery
`

type SetCommentDesignDeliveryParams struct {
	DesignDelivery []byte      `json:"design_delivery"`
	ID             pgtype.UUID `json:"id"`
	WorkspaceID    pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) SetCommentDesignDelivery(ctx context.Context, arg SetCommentDesignDeliveryParams) (Comment, error) {
	row := q.db.QueryRow(ctx, setCommentDesignDelivery, arg.DesignDelivery, arg.ID, arg.WorkspaceID)
	var i Comment
	err := row.Scan(
		&i.ID,
		&i.IssueID,
