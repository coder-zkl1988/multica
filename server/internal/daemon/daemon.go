...
type terminalTaskReport struct {
	...
	packageReceipt *ProjectDesignSystemPackageReceipt
...
type Daemon struct {
	...
	// runUpdateFn executes the fork-release upgrade. Set to d.runUpdate by
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// from "starting" to "running" only after startup actually completed.
	d.ready.Store(true)
	d.logger.Debug("background loops launched (workspace-sync, task-wakeup, heartbeat, gc, auto-update, auth-lifecycle); health now reporting ready")
...
// runUpdate executes the fork-release upgrade against targetVersion and
// returns the human-readable output. The caller is responsible for the
...
func (d *Daemon) runUpdate(targetVersion string) (string, error) {
	d.logger.Info("updating CLI from fork release...", "target_version", targetVersion)
...
func (d *Daemon) reportAuthenticationExpired(taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	if err := d.client.FailTask(reportCtx, taskID, errAuthenticationExpired.Error(), result.SessionID, result.WorkDir, taskfailure.ReasonAuthenticationExpired.String(), false, ""); err != nil {
...
func (d *Daemon) reportTaskResult(ctx context.Context, taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	case "completed":
		...
		err := d.reportTerminalTask(ctx, terminalTaskReport{
			...
			artifacts:             result.ProjectDesignSystemArtifacts,
			packageReceipt:        result.ProjectDesignSystemPackage,
...
func (d *Daemon) reportTerminalTask(parentCtx context.Context, report terminalTaskReport) error {
	...
	case terminalTaskReportComplete:
		return d.client.CompleteTask(ctx, report.taskID, report.output, report.branchName, report.sessionID, report.workDir, report.artifacts, report.packageReceipt, report.sessionRolloutMissing, report.retiredSessionID)
