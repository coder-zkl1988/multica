...
func (d *Daemon) reportAuthenticationExpired(taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	if err := d.client.FailTask(reportCtx, taskID, errAuthenticationExpired.Error(), result.SessionID, result.WorkDir, result.BranchName, taskfailure.ReasonAuthenticationExpired.String(), result.SessionRolloutMissing, result.RetiredSessionID, result.DurableWorkDir); err != nil {
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	if designDocumentGrounding != nil {
		defer func() {
			...
			finalized, finalizeErr := d.finalizeDesignDocumentResultFromDaemon(ctx, task, taskResult)
			if finalizeErr != nil {
				taskResult.Status = "blocked"
				taskResult.Comment = "Design Document finalize failed: " + finalizeErr.Error()
				taskResult.FailureReason = finalizeFailureDesignDocumentAuditFailed
