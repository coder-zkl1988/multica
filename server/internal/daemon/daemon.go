...
func (d *Daemon) reportAuthenticationExpired(taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	if err := d.client.FailTask(reportCtx, taskID, errAuthenticationExpired.Error(), result.SessionID, result.WorkDir, taskfailure.ReasonAuthenticationExpired.String(), result.SessionRolloutMissing, result.RetiredSessionID); err != nil {
