...
func TestClientCompleteTaskOmitsProjectDesignSystemArtifactsWhenNil(t *testing.T) {
	...
	client := NewClient(srv.URL)
	if err := client.CompleteTask(context.Background(), "task-1", "done", "", "", "", nil, nil, false, ""); err != nil {
		t.Fatalf("complete task: %v", err)
	}
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			call: func(c *Client) error {
				return c.CompleteTask(context.Background(), "task-1", "done", "", "", "/tmp/wd", nil, nil, false, "POISONED-S")
			},
		},
		{
			name:     "fail",
...
func TestTerminalReportsOmitEmptyRetiredSessionID(t *testing.T) {
	...
	if err := NewClient(srv.URL).CompleteTask(context.Background(), "task-1", "done", "", "sess-1", "/tmp/wd", nil, nil, false, ""); err != nil {
