...
func buildMetaSkillContentFull(provider string, ctx TaskContextForEnv) string {
	...
	} else if ctx.DesignSystemProfileAnalyzeContext != "" {
		b.WriteString("**This task was triggered by a Figma UI specification upload.** Follow the embedded design system profile analysis context and the JSON-only output contract in the user message you just received; ignore the default assignment-task workflow.\n\n")
		b.WriteString("Hard guardrails:\n")
		b.WriteString("- Do NOT call `multica issue get`, `multica issue status`, or `multica issue comment add` for this task.\n")
