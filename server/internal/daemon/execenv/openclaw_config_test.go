...
Try: openclaw config get --help)`)},
	...
	got := mustReadJSON(t, result.ConfigPath)
	include, _ := got["$include"].([]any)
	// The wrapper includes the user's config and then the mcp-reset file, which
	// nulls the user's servers without this package ever parsing that config.
	if len(include) != 2 {
		t.Fatalf("$include = %v, want the user config plus the mcp reset", got["$include"])
	}
	reset := mustReadJSON(t, include[1].(string))
	resetMcp, _ := reset["mcp"].(map[string]any)
	if servers, present := resetMcp["servers"]; !present || servers != nil {
		t.Errorf("mcp reset does not null the user's servers: %v", reset)
	}
