...
// runAuth serves one request through the middleware in LEGACY mode, which is
// the mode the plain cookie sessions above belong to: this fork's SSO mode
// only accepts a token whose auth_source says "sso", so running these there
// would reject every fixture before renewal was ever reached.
// TestAuth_RenewalKeepsTheSSOAuthSource covers the SSO side.
func runAuth(t *testing.T, req *http.Request) (*httptest.ResponseRecorder, bool) {
	t.Helper()
	return serveAuth(t, Auth(nil, nil, nil, nil, false), req)
}

func serveAuth(t *testing.T, mw func(http.Handler) http.Handler, req *http.Request) (*httptest.ResponseRecorder, bool) {
	t.Helper()
	called := false
...
// An SSO deployment accepts a session only while its auth_source claim says
// "sso". A renewal that dropped the claim would hand the browser a cookie its
// own next request rejects — a silent logout halfway through every session.
func TestAuth_RenewalKeepsTheSSOAuthSource(t *testing.T) {
	ssoAuth := Auth(nil, nil, nil, nil, true)
	original := generateToken(jwt.MapClaims{
		"sub":         "test-user-id",
		"email":       "test@multica.ai",
		"auth_source": "sso",
		"sid":         "sid-sso",
	...
	rec, called := serveAuth(t, ssoAuth, cookieRequest(http.MethodGet, original))
	if !called {
		t.Fatalf("the SSO session must still be served (status %d)", rec.Code)
	}
	...
	next, served := serveAuth(t, ssoAuth, cookieRequest(http.MethodGet, renewed))
	if !served {
		t.Fatalf("the renewed cookie must authenticate on its own (status %d)", next.Code)
	}
}
