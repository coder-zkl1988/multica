...
func RenewSessionToken(claims jwt.MapClaims) (string, time.Time, error) {
	...
	// This fork's SSO deployments only accept a session whose auth_source says
	// "sso" (ParseInternalToken). A renewal that dropped the claim would mint a
	// cookie the very next request rejects, so the renewed session has to state
	// the same source the renewed one did.
	if source, ok := claims["auth_source"].(string); ok && strings.TrimSpace(source) != "" {
		next["auth_source"] = source
	}
