...
func seedStatusCategoryFixture(t *testing.T) (projectID, customKey string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, $2, $3)
		RETURNING id
	`, testWorkspaceID, fmt.Sprintf("Status category %d", suffix), testUserID).Scan(&projectID); err != nil {
...
func TestIssueTableStatusCategoryFoldsBuiltInsWithoutCustomStatuses(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO project (workspace_id, title, created_by) VALUES ($1, $2, $3) RETURNING id
	`, testWorkspaceID, fmt.Sprintf("Status category parity %d", suffix), testUserID).Scan(&projectID); err != nil {
		t.Fatalf("create project: %v", err)
	}
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM issue WHERE project_id = $1`, projectID)
