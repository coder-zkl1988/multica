...
func TestStatusCompatibilityContractedStorageGroupsAndPagination(t *testing.T) {
	for _, storage := range []string{"contracted"} {
		t.Run(storage, func(t *testing.T) {
			project := dbfx.Insert(t, "project", testutil.Cols{"workspace_id": testWorkspaceID, "title": "Compatibility " + storage, "created_by": testUserID})
...
func TestStatusCompatibilityParentLaneUsesExpandedVisibleKeys(t *testing.T) {
	project := dbfx.Insert(t, "project", testutil.Cols{"workspace_id": testWorkspaceID, "title": "Category parent lane", "created_by": testUserID})
