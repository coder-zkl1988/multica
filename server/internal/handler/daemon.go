...
func (h *Handler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	...
	if existingTask.Status == "running" && isRepositoryAnalysis {
		if analysisErr != nil {
			...
			failedTask, failErr := h.TaskService.FailTask(r.Context(), parseUUID(taskID), analysisErr.Error(), req.SessionID, req.WorkDir, req.BranchName, "project_design_system_repository_analysis_invalid_output", req.SessionRolloutMissing, req.RetiredSessionID)
			if failErr != nil {
		...
		if prepareErr != nil {
			failedTask, failErr := h.TaskService.FailTask(r.Context(), existingTask.ID, prepareErr.Error(), req.SessionID, req.WorkDir, req.BranchName, "project_design_system_invalid_artifacts", req.SessionRolloutMissing, req.RetiredSessionID)
			if failErr != nil {
	...
	if existingTask.Status == "running" && isDesignTemplateBlueprintAnalyzeTaskContext(existingTask.Context) {
		...
		if blueprintErr != nil {
			...
			failedTask, failErr := h.TaskService.FailTask(r.Context(), parseUUID(taskID), blueprintErr.Error(), req.SessionID, req.WorkDir, req.BranchName, "design_template_blueprint_invalid_output", req.SessionRolloutMissing, req.RetiredSessionID)
			if failErr != nil {
