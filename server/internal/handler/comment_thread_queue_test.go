...
func TestCommentThreadQueuesMergeAndExecuteIndependently(t *testing.T) {
	...
	enqueue := func(commentID string, pending bool, want DispatchStatus) {
		...
		result := testHandler.enqueueCommentAgentTriggers(ctx, issue, parseUUID(commentID),
			[]commentAgentTrigger{{Agent: agent, Source: commentTriggerSourceMentionAgent, AlreadyPending: pending}}, false)
	...
	rerun, err := testHandler.TaskService.RerunIssue(ctx, issue.ID, a.ID, pgtype.UUID{}, parseUUID(testUserID), nil, nil)
	if err != nil {
		t.Fatal(err)
	}
