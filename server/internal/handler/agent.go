...
type AgentTaskResponse struct {
	...
	DesignDeliveryContext             json.RawMessage        `json:"design_delivery_context,omitempty"`                   // a saved design document delivered to this task's issue, for the agent implementing it (DC-062)
	...
	Kind           string `json:"kind"`                      // discriminator: comment | autopilot | chat | quick_create | design_restore | design_system_profile_analyze | direct
...
// (no linked source — the agent is creating the issue itself) / typed design
// contexts / direct (assignee-driven task on an existing issue).
