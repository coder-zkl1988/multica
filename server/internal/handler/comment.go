...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	if authorType == "agent" && sourceTaskID.Valid {
		managed, createErr := h.createManagedTaskComment(r.Context(), createParams, len(attachmentIDs) > 0)
		err = createErr
	...
	// The transaction above links a member's attachments inside the same
	// commit; a managed agent comment is already committed, so its
	// attachments are linked here.
	if authorType == "agent" && sourceTaskID.Valid && !reusedManagedFinal && len(attachmentIDs) > 0 {
		if linkErr := h.Queries.LinkAttachmentsToComment(r.Context(), db.LinkAttachmentsToCommentParams{
			CommentID: comment.ID,
			IssueID:   issue.ID,
			Column3:   attachmentIDs,
		}); linkErr != nil {
			slog.Error("failed to link attachments to comment", "error", linkErr)
		}
	}
...
// deferred / blocked from enqueue. UI-suppressed triggers produce no outcome.
// conciseMode opts freshly enqueued runs into the lightweight task prompt; a
...
func (h *Handler) enqueueCommentAgentTriggers(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, triggers []commentAgentTrigger, conciseMode bool) map[string]commentEnqueueResult {
	...
	for _, trigger := range triggers {
		status, reason := h.resolveCommentTriggerEnqueue(ctx, issue, trigger, triggerCommentID, conciseMode)
...
func (h *Handler) resolveCommentTriggerEnqueue(ctx context.Context, issue db.Issue, trigger commentAgentTrigger, triggerCommentID pgtype.UUID, conciseMode bool) (DispatchStatus, DispatchReasonCode) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err := h.enqueueSingleCommentTrigger(ctx, issue, triggerCommentID, trigger, conciseMode); err != nil {
...
func (h *Handler) propagateUncoveredCommentObligation(ctx context.Context, issue db.Issue, trigger commentAgentTrigger, commentID pgtype.UUID, headSha pgtype.Text) bool {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		err = h.enqueueSingleCommentTrigger(ctx, issue, commentID, trigger, false)
		if err == nil {
			return true
		}
		if !errors.Is(err, service.ErrDuplicatePendingTask) {
...
// (MUL-4195) can fall back to it when a pending task vanished mid-flight. It
// returns the PRIMARY enqueue error (nil on success) so the caller can surface
// a trigger_outcome (MUL-4525 §2).
func (h *Handler) enqueueSingleCommentTrigger(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, trigger commentAgentTrigger, conciseMode bool) error {
	switch trigger.Source {
	case commentTriggerSourceIssueAssignee:
		if trigger.Squad != nil {
			if _, err := h.TaskService.EnqueueTaskForSquadLeaderWithMode(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, service.OriginDerived, conciseMode); err != nil {
		...
		if _, err := h.TaskService.EnqueueTaskForIssueWithMode(ctx, issue, triggerCommentID, service.OriginDerived, conciseMode); err != nil {
	...
	case commentTriggerSourceMentionSquadLeader:
		if _, err := h.TaskService.EnqueueTaskForSquadLeaderWithMode(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, service.OriginNamed, conciseMode); err != nil {
	...
	case commentTriggerSourceMentionAgent:
		if _, err := h.TaskService.EnqueueTaskForMentionWithMode(ctx, issue, trigger.Agent.ID, triggerCommentID, service.OriginNamed, conciseMode); err != nil {
	...
	case commentTriggerSourceThreadParent, commentTriggerSourceConversation:
		...
		if trigger.Squad != nil {
			_, err = h.TaskService.EnqueueTaskForSquadLeaderWithMode(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, service.OriginNamed, conciseMode)
		} else {
			_, err = h.TaskService.EnqueueTaskForThreadParentWithMode(ctx, issue, trigger.Agent.ID, triggerCommentID, conciseMode)
		}
		if err != nil {
