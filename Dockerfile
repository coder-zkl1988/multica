...
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${COMMIT} -X main.upstreamVersion=${UPSTREAM_VERSION}" -o bin/server ./cmd/server
