 ...
 * CUSTOM statuses reach those surfaces through the catalog, not this list.
 */
...
/**
 * Built-in statuses are translated; a CUSTOM status has no translation key, so
 * it falls back to the catalog name the server sent (and to the raw key when
 * there is none). That ordering is what lets MUL-6243's custom statuses and
 * this app's localisation coexist: neither one blanks the chip.
 */
export function issueStatusLabel(
  ...
  return t(`status.${value}`, { defaultValue: fallbackName ?? value });
}
...
/** `statusOptions` output with each built-in label run through i18n. */
export function translatedStatusOptions(
  t: IssueTranslate,
  catalog: IssueStatusCatalog,
): StatusOption[] {
  return statusOptions(catalog).map((option) => ({
    ...option,
    label: issueStatusLabel(t, option.key, option.label),
  }));
}
