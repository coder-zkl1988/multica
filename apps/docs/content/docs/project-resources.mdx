...
description: Link GitHub repositories, local directories, or external documents to a project so later runs get a stable working context.
---
...
| **Document** | PRDs, API references, business-rule pages, or any specification the agent should read before starting a task | Not applicable — the agent fetches the URL |
...
A document resource pins an external page — a PRD, API spec, or business-rule guide — as durable project context. The agent fetches and reads the URL before implementing or testing anything the specification describes.

1. Open the project's **Resources**.
2. Choose "Add document".
3. Enter the URL (must start with `http://` or `https://`), a title, and an optional summary.

The URL is validated at creation time; relative paths and non-HTTP schemes are rejected. The title is required. The optional summary gives the agent a short description of what the document covers; it appears in the agent's brief alongside the URL.

Agents treat document resources as carrying the same authority as code. When a task involves a feature or API the document describes, the agent is expected to check the document first.
...
# Link a document (PRD, API spec, business-rule page)
multica project resource add <project-id> \
  --type document \
...
multica project resource add <project-id> \
  ...
  --title "OpenAPI 3 Schema" \
  --summary "Authoritative API contract for the billing service."
