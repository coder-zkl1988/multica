...
Auto-update is on by default. The app checks for and downloads new versions in the background, with a persistent progress card showing the current percentage. Repeated checks while that download is active reuse the same transfer rather than starting another one. Once the download finishes you can restart to install immediately or let it install on the next quit. You can also turn off automatic checks or check for new versions manually under **Settings → Updates**.
