---
title: 项目资源
description: 为项目关联 GitHub 仓库、本地目录或外部文档，让后续执行获得稳定的工作上下文。
---
...
| **文档** | PRD、API 文档、业务规则页面，或智能体在开始任务前需要阅读的任何规格说明 | 不适用——智能体会直接请求该 URL |
...
文档资源将一个外部页面——PRD、API 规格、业务规则说明——作为持久项目上下文固定下来。智能体在实现或测试规格中描述的任何内容之前，会先拉取并阅读该 URL。

1. 打开项目的 **资源**。
2. 选择"添加文档"。
3. 填入 URL（必须以 `http://` 或 `https://` 开头）、标题（必填）和可选的摘要。

URL 在创建时会做校验，相对路径和非 HTTP 协议会被拒绝。摘要字段最多 500 个字符，给智能体一段对文档内容的简短说明，会和 URL 一起出现在智能体的执行上下文中。

智能体会把文档资源视为与代码同等权威的来源。任务涉及文档中描述的功能或 API 时，智能体会先查阅文档再动手。
