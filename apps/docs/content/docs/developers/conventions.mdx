...
A standalone nav-row or section-header label inside product UI (e.g. a mobile "More" page's "Issues" row, a sidebar section title) is a **UI string**, not a doc title/heading — even though it looks similarly bare. It follows the "UI strings" row above (lowercase English: "issue"/"issues"), not the "Doc titles" row, since it isn't a page/doc heading. This distinction is what the 2026-07 mobile More-page review caught: nav rows had been capitalized "Issues" as if they were doc-style headings.

Both concepts are user-visible and they are **not** the same thing — an issue can carry many runs. Never let one locale spell both with the same word, which is why the run keeps a visually distinct form in each locale. `Task` is no longer a user-facing product noun for an agent run; it remains only as the established internal identifier.
