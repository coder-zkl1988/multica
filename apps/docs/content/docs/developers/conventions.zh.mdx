...
产品 UI 里独立出现的导航行 / 分组标题（例如移动端「更多」页的 "Issues" 行、侧边栏分组标题）属于**UI 短句**，不是 doc 标题/章节标题 —— 虽然外观上都很「独立一行」，但它不是文档页面标题，应套用上面「UI 短句」那一行（小写英文 "issue"/"issues"），而不是「doc 标题」那一行。2026-07 移动端「更多」页评审就踩过这个坑：导航行曾被当成 doc 风格标题大写成了 "Issues"。

这两个概念用户都看得到，而且**不是一回事** —— 一个任务下面可以有多次运行。任何一种语言都不能把两者写成同一个词，所以「执行」在每种语言里都保留一个视觉上区分得开的写法。`Task` 不再是智能体单次运行的用户侧产品名，只保留为已经稳定的内部标识符。
