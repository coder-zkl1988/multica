...
- 替代原因：2026-09-08 用户确认 UI Kit 自身应承载导航和内容，外层章节目录会与包内导航重复；由 DC-063 替代。
...
- 替代原因：2026-09-08 用户确认默认主视图应为包内完整 UI Kit，文档和 Tokens 退到次级入口；由 DC-063 替代。
...
### DC-063 仓库设计体系采用统一的 Open Design 生成规则

- 状态：`confirmed`
- 日期：2026-09-08
- 决策：所有仓库统一使用 Open Design v0.19.2 的仓库分析与设计体系生成规则。Multica 只适配自己的仓库、Agent、对象存储、权限及 saved/draft 生命周期，不为 CRM 或任何单一仓库增加专用生成规则。
- 仓库边界：设计 Token、组件、页面模式和业务扩展必须从当前仓库默认分支及仓库自身资料中提取；禁止在通用 Prompt、提取器、UI Kit 模板或 Audit 中写死特定仓库的值。
- 外部对照：用于人工验收的站点、截图或快照不自动进入 Agent 输入，也不构成全局生成权威；只有用户在具体生成任务中明确添加的参考资料才属于该任务上下文。
- 生成链路：保留完整仓库 checkout、完整 Base Archive、Agent 原地深化、Package Audit、Preview/UI Kit 和 draft/saved 隔离。
- UI Kit：设计体系默认主视图为包内 UI Kit，内部导航与内容同版本；`DESIGN.md`、Tokens 和包文件是次级入口。
- 替代关系：本决策替代 DC-031 的外层章节目录和 DC-033 的文档连续画布，但保留其“同 Tab 直接展示、调整面板按需出现、内容为主角”的产品边界。

### DC-064 任务评论按次发起设计交付

- 状态：`confirmed`（SY-323 产品讨论；实现检查不等于真实智能体最终验收）
- 日期：2026-09-08
- 唯一入口：只在任务评论编辑器提供“设计交付”，不在属性侧栏、正文或新建任务编辑器提供入口。UI设计和UI还原属于本次交付，不是任务永久类型，也没有强制设计阶段或按标题推导的完成门槛。
- 回复入口：动态线程里的回复输入框与底部评论输入框提供同样的设计交付配置与发送行为；仅配置不发起，发送保留线程归属，草稿与其他线程隔离。
- 发起：用户选择当前项目的仓库、本次执行智能体，填写要求并发送评论后才开始。智能体默认可以来自任务负责人，但可更换且不自动改派任务；普通讨论、正文保存和编辑已发起的评论不重新发起设计交付。重试同一次发送不重复创建任务和设计稿。
- 输入：当前评论明确本轮要求，任务正文作为背景；设计体系可选，显式不指定时不隐式回落默认体系。此边界覆盖早期“必须先保存仓库体系才能设计”的交付限制。
- 无体系时的视觉保真：已选择仓库但本轮不使用设计体系时，默认忠实沿用仓库实际生效的视觉规范。追踪全局样式入口、变量消费关系、布局与共享组件；不把单个声明值或目录列表当作生效证据。对比度等优化只提示并提出建议，未经用户明确同意，不自行改色、改尺寸或重设计导航。无法确定适用规则时说明缺口，不伪造继承关系；不隐式加载 saved 体系、不增加仓库专用规则。
- 观察：发起评论下展示本次任务的真实工作台快照，只供查看。运行中的快照不是正式修订，不移动 draft/saved，也不跳过最终 Audit/Preview。完成后点击进入同一设计稿的完整工作台；调整旧稿在完整工作台进行，评论设计入口发起新稿。
- 卡片进展与比例：使用本次任务真实回流的公开计划展示阶段与已完成步骤，不编造百分比、不展示思考内容。预览保持验证视口的画布尺寸，按卡片宽度等比缩小，不因卡片变窄触发响应式重排；实时快照暂无视口元数据时沿用浏览器门禁默认视口，最终预览使用回执视口。
- 归属：稿件关联项目、本次项目仓库及来源任务；已有合法工作区仓库关联时，两种视角打开同一稿件，不复制资产。
- 还原：复用统一 Figma/Multica 引用、精确页面/Frame 和 MCP 分流；在本次线程交付结构化结果。预览证据的相对文件路径不是网页地址；仅明确提供并通过校验的 HTTP(S) 预览 URL 可以作为运行页面入口，回环地址标记仅本机可访问。没有 URL 时如实提示不可用，不替换成源设计稿预览，不自动部署。
- 实现边界：已有评论和任务权限、调用权限、不可变输入及工作区隔离继续有效；设计交付不能额外唤醒未选择的智能体。真实智能体生成质量和用户最终验收仍需在目标环境确认。
- 实施证据（2026-09-08）：动态回复提交链路、卡片进展与等比预览已接入；页面设计 checkout 回执携带无体系时的视觉保真指令，未修改 Open Design 共享生成引擎。定向回归、TypeScript 与隔离 Chromium 组件布局核查不替代真实目标仓库生成质量验收；旧设计稿及 saved 不自动改写。

### DC-065 生成设计稿采用统一的浏览与交付展示

- 状态：`confirmed`（产品边界；逐项实施和验收）
- 日期：2026-09-09
- 来源：首页创作与 Issue UI设计先产生草稿，再由用户保存为正式稿；Figma 上传保持原有设计稿入口。复用 Figma 的页面总览、单页详情展示，不转换文件格式或改变 Figma 保存流程。
- 归属：项目／仓库展示同一份资产。有未保存调整时，正式稿入口展示 saved 并提示“有未保存调整”，草稿入口打开工作台中的新草稿，不复制稿件。
- 版本：继续调整回原工作台，成功后产生新修订草稿；显式保存才移动正式版本及封面。历史版本保留，已经发出的实现任务引用不自动改变。
- 缩略图：首页作为对应版本默认封面，显示页数；页面总览展示每页真实缩略图。暂不手动选封面，不用实时运行快照覆盖已保存版本封面。
- 单页详情：默认查看模式允许原型交互；检查模式点击只选取元素、查看属性，不触发页面操作。修改仍回工作台。
- 素材：只下载当前稿件已有图片／图标，不提供任意区域切图或倍率导出。
- MCP：第一阶段交付单页或关联多页，固定已保存版本；局部元素交付后续讨论。
- 实施顺序：入口与版本隔离 → 缩略图、页数与页面总览 → 单页检查与素材下载 → 单页／多页交付闭环；依原任务要求逐项验收，不把本项实现当成全部完成。
- 第一项实现：正式稿列表打开文档 `/view` 只读页面，只读取 saved 指针；草稿和首页最近生成仍打开原工作台。无 saved 或加载失败不回落草稿。已完成定向组件／导航回归及类型检查；浏览器真实 UI 验收、后续各项仍待完成。
- 第二项实现：生成稿卡片按 saved/draft 的明确指针读取版本，生成静态 PNG 封面并展示页数；只读页先展示每页缩略图总览，点击进入对应页面，可返回总览。缩略图按修订、内容摘要、页面和验证视口缓存，延迟生成、串行渲染并限制缓存数量；缺失资源显示失败，不伪造图像。缓存仅在当前浏览器会话内，不新增持久化服务。
- 第二项验证：Chromium 中实际执行内联及栅格化链路，得到 512×360 PNG；同版本复用缓存，换版本得到不同颜色像素，原型脚本未执行。修复既有 foreignObject 使用 blob SVG 导致画布污染的问题。定向组件回归与类型检查通过，真实项目视觉验收仍需进行。缩略图是脚本禁用的静态初始视图；仅靠脚本绘制的图表／画布不会等同于交互运行结果，此限制不作为动态渲染能力已完成。
- 第三项实现：正式稿单页默认交互查看，可切换脚本禁用的只读检查，点击选取元素查看计算后的尺寸、间距、颜色、字体并复制样式；切换页面／修订清空选取。沿用静态画布并修复 iframe 跨 realm 元素判断。已有图片／图标来自当前修订的文件索引，通过该修订资源路由下载原文件，校验路径、MIME 和图片解码；切换修订取消旧下载。无区域切图、倍率导出或 MCP 改动。
- 第三项验证：定向组件测试覆盖 iframe 点击选择／阻止导航、素材安全校验、失败重试和旧修订下载竞态；Chromium 合成页面实际得到 12px padding、20px 字号和绿色文本，点击未执行原型脚本，修订切换清空属性；SVG 预览解码成功且下载字节保持一致。该验证不替代部署后真实项目页面的视觉验收，运行服务未更新。
- 第三项交互修订：用户明确要求贴合 Figma 详情，而非独立 CSS 属性列表。现改为固定比例左画布、380px 独立滚动检查栏、分组属性卡片、底部缩放／翻页工具条；查看／检查保留同一布局，素材归入右栏。使用 DOM 实际属性，不伪造 Figma 节点或加入 JSON 编辑。原第三项未通过用户视觉验收，第四项继续暂停。
- 修订验证与发布：17 项定向测试及类型检查通过，独立生产构建成功。带真实构建样式的 Chromium 合成页面确认选取联动、只读边界、缩放不重排、翻页清空选取与查看模式交互；已将 SY-323 前端切换到 web-figma-detail 发布目录，公开入口新资源校验一致。实际稿件视觉验收仍由用户确认，不将合成页面验证当作已验收。
