...
11. **设计体系内容主视图**，`confirmed`：设计体系 Tab 默认直接展示包内完整在线 UI Kit，由 UI Kit 内部承载基础、Tokens、通用组件、页面模式和验收导航；Multica 外层只保留状态、版本、调整、重新生成、保存和设计文件入口，不重复维护章节目录。
12. **设计体系创建工作台**，`confirmed`：未建立体系时直接在设计体系 Tab 内展示自然语言为主的单屏创建工作台；目标平台和执行智能体必选，其他参考资料可选，提交后原地进入生成状态。
13. **设计体系 UI Kit 主画布**，`confirmed`：草稿和已保存状态默认共用包内 UI Kit 全画布；`DESIGN.md`、Tokens 和包文件退到“设计文件”次级入口。智能体调整面板默认关闭，只有显式进入选择调整或全局调整时出现。
...
31. **仓库设计体系使用统一生成链路**，`confirmed`（详见 DC-063）：所有仓库均使用同一套 Open Design 生成规则；仓库自身的默认分支代码与仓库内资料决定该仓库的设计体系，不写入任何特定仓库的颜色、尺寸、组件或页面规则。
